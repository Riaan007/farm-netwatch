    ...
    function drawDetail() {
      ...
      el.innerHTML = `<div class="swv-box swv-detail" style="border-color:rgba(34,211,238,.35)">
        <div class="swv-head"><div><h2 style="font-size:17px">Port ${esc(portNo(p))}${!isDefaultName(p.name) ? ` · ${esc(p.name)}` : ""} <span class="swv-b ${s.cls}">${esc(s.label)}</span></h2>
          <p class="swv-muted">${p.up ? `${p.speed ? p.speed + " Mbit/s " + (p.duplex || "") : "link up"} · ` : ""}${p.poe_supported ? "PoE " + esc(poeLabel(p.poe_mode)) + ((p.poe_w || 0) >= 0.5 ? ` drawing ${(+p.poe_w).toFixed(1)} W` : "") : "no PoE"}</p></div>
        ...
        <div class="swv-grid2">
          ...
          <div><div class="swv-note">PoE power (W)</div>${!p.poe_supported ? `<p class="swv-note">This port has no PoE.</p>` : rows ? lineChart(rows, [{ k: "poe_w", c: "#fde68a", label: "watts" }], { fmt: (v) => v.toFixed(1) + " W", aria: "PoE draw", emptyNote: p.poe_mode === "off" ? "PoE is switched off on this port." : "Nothing drew PoE power on this port in this period." }) : `<div class="swv-skel" style="height:120px"></div>`}</div>
        </div>
        <div class="swv-grid2">
